...
public final class Util {
    ...
    /**
     * <p> This method returns the ContextClassLoader unless it is null, in which case it returns the ClassLoader that
     *     loaded "obj". Unless it is null, in which it will return the system ClassLoader.</p>
     *
     * @param obj   May be null, if non-null when the Context ClassLoader is null, then the Classloader used to load
...
// NOTE: Maybe in addition to getClassLoader, we should have Iterator<ClassLoader> getClassLoaders() for cases where we want to attempt multiple ClassLoaders
    ...
    /**
     * <p> Method to get the custom <code>ClassLoader</code> if one exists. If one does not exist, it will return the
     *     <code>ClassLoader</code> that is passed in. If (null) is passed in for the parent
     *     <code>ClassLoader</code>, it will get the <b>System</b> <code>ClassLoader</code>, not the context
	...
	parent = (parent == null) ? ClassLoader.getSystemClassLoader() : parent;
        ...
        final Map<ClassLoader, ClassLoader> classLoaderCache = getClassLoaderCache(ctx);
...
//                Thread.currentThread().setContextClassLoader(loader);
            ...
            } catch (final InvocationTargetException | InstantiationException ex) {
    ...
    /**
     * <p> Provides access to the application-scoped Map which stores custom ClassLoaders which may wrap a parent
     *     ClassLoader in this application.</p>
     */
    @SuppressWarnings("unchecked")
    private static Map<ClassLoader, ClassLoader> getClassLoaderCache(final FacesContext facesContext) {
        final FacesContext ctx = (facesContext == null) ? FacesContext.getCurrentInstance() : facesContext;
    ...
    /**
     * <p> This method will attempt to load a Class from the context ClassLoader. If it fails, it will try from the
     *     ClassLoader used to load the given object. If that's null, or fails, it will try using the System
     *     ClassLoader.</p>
     *
     * @param className The full name of the class to load.
     * @param obj       An optional Object used to help find the ClassLoader to use.
     */
    public static Class<?> loadClass(final String className, final Object obj) throws ClassNotFoundException {
        ...
        if (cls == null) {
            ...
            try {
                cls = (loader == null) ? null : Class.forName(className, false, loader);
            } catch (ClassNotFoundException ex) {
                // Ignore
                if (LogUtil.finestEnabled()) {
                    LogUtil.finest("Unable to find class (" + className
    ...
    /**
     * <p> Method which returns the Class for the given class name, or null if any exception occurs. No exceptions
     *     are thrown.</p>
     */
    public static Class<?> noExceptionLoadClass(final String name) {
        try {
            return Util.loadClass(name, null);
    ...
    /**
     * <p> This method attempts load the requested Class. If obj is a String, it will use this value as the fully
     *     qualified class name. If it is a Class, it will return it. If it is anything else, it will return the
     *     Class for the given Object.</p>
     *
     * @param obj   The Object describing the requested Class.
     * @return the class.
     */
    public static Class<?> getClass(final Object obj) throws ClassNotFoundException {
    ...
    /**
     * <p> This method locates the requested <code>Method</code> on the given <code>Class</code>, with the given
    ...
    /**
     * <p> This method escapes text so that HTML tags and escape characters can be shown in an HTML page without
     *     seeming to be parsed.</p>
     */
    ...
    /**
     * <p> This method reverses html-escaping in the given String. It is meant to be the converse of
     *     {@link #htmlEscape(String)}. It does not convert every entity, it only converts those supported by
